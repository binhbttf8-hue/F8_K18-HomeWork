...
// Helper: lấy toàn bộ data từ form
...
// Helper: map field → error element id
const FIELD_MAP = [
...
// Helper: áp kết quả safeParse lên UI
function applyValidationResult(result, touchedFields = null) {
    ...
    FIELD_MAP.forEach(({ field, error }) => {
        // Bỏ qua field chưa được chạm vào (khi validate real-time)
...
// ══════════════════════════════════════════════════════════════
// TODO 5: Toggle hiển thị mật khẩu
// ══════════════════════════════════════════════════════════════
...
document.getElementById("password").addEventListener("input", () => {
    ...
    let score = 0;
    if (val.length >= 8)            score++; // độ dài ≥ 8
    if (/[A-Z]/.test(val))          score++; // có chữ hoa
    if (/\d/.test(val))             score++; // có số
